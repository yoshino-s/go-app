...
func New(opts ...Option) *OtlpApp {
	app := &OtlpApp{
		EmptyApplication: application.NewEmptyApplication("otlp"),
	}
...
func (app *OtlpApp) Setup(ctx context.Context) {
	...
	if err != nil {
		app.Logger.Sugar().Panicf("invalid Uptrace DSN: %s (Uptrace is disabled)", err)
		return
	}
	app.dsn = dsn
	...
	if app.config.tracingEnabled {
		...
		if err != nil {
			app.Logger.Error("configureTracing failed", zap.Error(err))
		}
	...
	if app.config.metricsEnabled {
		...
		if err != nil {
			app.Logger.Error("configureMetrics failed", zap.Error(err))
		}
	...
	if app.config.loggingEnabled {
		...
		if err != nil {
			app.Logger.Error("configureLogging failed", zap.Error(err))
		}
...
func (c *OtlpApp) Close(ctx context.Context) {
	if c.TraceProvider != nil {
		if err := c.TraceProvider.Shutdown(ctx); err != nil {
			c.Logger.Error("TraceProvider Shutdown failed", zap.Error(err))
		}
		c.TraceProvider = nil
	}
	if c.MeterProvider != nil {
		if err := c.MeterProvider.Shutdown(ctx); err != nil {
			c.Logger.Error("MeterProvider Shutdown failed", zap.Error(err))
		}
		c.MeterProvider = nil
	}
	if c.LoggerProvider != nil {
		if err := c.LoggerProvider.Shutdown(ctx); err != nil {
			c.Logger.Error("LoggerProvider Shutdown failed", zap.Error(err))
		}
